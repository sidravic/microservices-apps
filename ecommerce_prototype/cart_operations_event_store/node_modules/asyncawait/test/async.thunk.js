...
describe('A suspendable function returned by async.thunk(...)', function () {
    ...
    it('does not execute if the thunk is not invoked', function (done) {
        ...
        var thunk = foo();
        Promise.delay(50).then(function () { return expect(x).to.equal(5); }).then(function () { return done(); }).catch(done);
        expect(x).to.equal(5);
    });
    it('executes if the thunk is invoked without a callback', function (done) {
        ...
        foo()();
        Promise.delay(20).then(function (result) { return expect(x).to.equal(7); }).then(function () { return done(); }).catch(done);
    });
    it('begins executing synchronously and completes asynchronously', function (done) {
        ...
        Promise.promisify(foo())().then(function () { return expect(x).to.equal(9); }).then(function () { return done(); }).catch(done);
        expect(x).to.equal(7);
        x = 9;
    });
    it("preserves the 'this' context of the call", function (done) {
        ...
        Promise.promisify(foo.bar.call(foo))().then(function (result) { return expect(result).to.equal(foo); }).then(function () { return Promise.promisify(foo.bar.call(baz))(); }).then(function (result) { return expect(result).to.equal(baz); }).then(function () { return done(); }).catch(done);
    });
    it('eventually resolves with its definition\'s returned value', function (done) {
        var foo = async.thunk(function () {
            return 'blah';
        });
        Promise.promisify(foo())().then(function (result) { return expect(result).to.equal('blah'); }).then(function () { return done(); }).catch(done);
    });
    it('eventually rejects with its definition\'s thrown value', function (done) {
        ...
        Promise.promisify(foo())().catch(function (err) { return act = err; }).then(function () {
            if (!act)
    ...
    it('works with await', function (done) {
        var foo = async.thunk(function () {
            return await(Promise.delay(20).then(function () { return 'blah'; }));
        });
        Promise.promisify(foo())().then(function (result) { return expect(result).to.equal('blah'); }).then(function () { return done(); }).catch(done);
    });
