...
describe('A suspendable function returned by async.cps(...)', function () {
    ...
    it("preserves the 'this' context of the call", function (done) {
        ...
        Promise.promisify(foo.bar.bind(foo))().then(function (result) { return expect(result).to.equal(foo); }).then(function () { return Promise.promisify(foo.bar).call(baz); }).then(function (result) { return expect(result).to.equal(baz); }).then(function () { return done(); }).catch(done);
    });
